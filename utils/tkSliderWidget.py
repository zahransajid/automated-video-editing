...
class Slider(Frame):
    ...
    def __init__(
        ...
        self.canv = Canvas(self, height=self.canv_H, width=self.canv_W+25)
        self.canv.pack()
    ...
    def __addBar(self, pos):
        ...
        if self.show_value:
            ...
            hrs, mins, secs = self.get_time(value)
            id_value = self.canv.create_text(
                x+15, y_value, text=f"{int(hrs):02d}:{int(mins):02d}:{int(secs):02d}"
            )
